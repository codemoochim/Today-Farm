...
class DB {
  ...
  async insertData({ deviceId, temp, humid, lux, solid, time }) {
    const sql = `INSERT INTO datas (deviceId, temp, humid, lux, solid, time) value (?,?,?,?,?,?)`;
    const [rows] = await this.promisePool.query(sql, [deviceId, temp, humid, lux, solid, time]);
    return rows;
  }
